...
Sự phát triển bùng nổ của các nền tảng mạng xã hội đã thay đổi hoàn toàn thói quen giao tiếp và cập nhật thông tin của giới trẻ. Đối với cộng đồng sinh viên tại các trường đại học, không gian mạng không chỉ là nơi giải trí mà còn là môi trường trao đổi học thuật chủ đạo. Mặc dù vậy, các mạng xã hội thống lĩnh thị trường hiện tại (như Facebook, Instagram hay X) chủ yếu được thiết kế để duy trì những mối quan hệ đã được thiết lập từ trước, hoặc phân phối nội dung định hướng dựa trên mức độ tương tác trực tuyến của người dùng. \cite{tac-dong-cua-mang-xa-hoi}

Cơ chế này vô tình tạo ra một nghịch lý: sinh viên có thể kết nối với thế giới rất nhanh chóng trên không gian mạng ảo, nhưng lại gặp khó khăn trong việc tìm kiếm và giao lưu với những người bạn mới có cùng sở thích chuyên sâu, cùng định hướng chuyên môn ngay tại môi trường học đường thực tế.
...
\subsubsection{Phạm vi hoạt động}
...
\subsubsection{Các loại hình hoạt động}
...
\begin{itemize}
    ...
    \item Hoạt động ngoại khóa và giải trí: Các buổi sinh hoạt của câu lạc bộ, các buổi giao lưu âm nhạc tự phát hoặc hẹn gặp gỡ giao lưu bạn mới có cùng sở thích.
\end{itemize}

\subsubsection{Tính năng cốt lõi}

Quản lý tài khoản và Hồ sơ sinh viên (User \& Profile Management): Cho phép đăng ký, định danh, và quản lý thông tin cá nhân. Đặc biệt, hệ thống sẽ thu thập các dữ liệu phi cấu trúc (ngành học, kỹ năng, sở thích) để làm đầu vào cho mô hình ngôn ngữ chuyển đổi thành các vector (embeddings).

Quản lý hoạt động và Tương tác không gian (Activity \& Spatial Management): Xử lý logic tạo mới, chỉnh sửa và quản lý vòng đời của một hoạt động. Cung cấp API để đánh dấu tọa độ lên bản đồ số (Location-based). Tích hợp sẵn không gian lưu trữ cục bộ cho từng hoạt động, cho phép các thành viên tham gia có thể đăng tải tài liệu (file đính kèm) và nhắn tin/thảo luận (chat/comment) trực tiếp trong phạm vi sự kiện đó.
...
Trợ lý ảo thông minh (AI Chatbot): Cung cấp giao diện giao tiếp tự nhiên bằng văn bản, giúp người dùng tra cứu nhanh các thông tin trên hệ thống (tìm sự kiện, hỏi về tài liệu đã chia sẻ, hướng dẫn tham gia hoạt động) thay vì phải sử dụng bộ lọc tìm kiếm truyền thống.

\subsubsection{Chatbot hỏi đáp}
...
\begin{itemize}
    \item Hỗ trợ thông tin và gợi ý: Trực tiếp phân tích câu hỏi của người dùng để truy xuất, đề xuất các hoạt động, sự kiện, nhóm học tập hoặc các tài liệu đang lưu hành trên hệ thống UniConnect sao cho khớp với vị trí và thời gian thực.
...
\subsubsection{Pháp lý và đạo đức}
...
\subsubsection{Ý nghĩa thực tiễn đối với cộng đồng và xã hội}
...
\subsubsection{Ý nghĩa đối với bản thân sinh viên thực hiện}

Tổng hợp và vận dụng toàn diện kiến thức: Quá trình thực hiện đồ án là cơ hội quý báu để em xâu chuỗi và áp dụng thực tế những kiến thức đã được đào tạo trong suốt 4 năm học viện. Từ tư duy thiết kế hệ thống, phân tích cơ sở dữ liệu, lập trình thuật toán (Python, Vector Search) cho đến việc xây dựng giao diện tương tác (React.js) và triển khai vận hành (Docker).

Trang bị kỹ năng giải quyết bài toán thực tế: Khác với các bài tập môn học có sẵn dữ liệu và khuôn mẫu, việc tự tay xây dựng UniConnect từ con số không đã rèn luyện cho em khả năng phân tích yêu cầu người dùng, xử lý các lỗi hệ thống phức tạp (bugs) và tư duy phát triển sản phẩm (Product Thinking). Đây là hành trang thực tiễn vô cùng vững chắc để em tự tin bước vào con đường kỹ sư phần mềm chuyên nghiệp sau khi tốt nghiệp.
