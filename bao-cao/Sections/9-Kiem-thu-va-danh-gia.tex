...
\section{Chiến lược và Quy trình kiểm thử}
Nhằm đảm bảo chất lượng, độ tin cậy và tính toàn vẹn của nền tảng UniConnect trước khi đưa vào vận hành thực tế, đồ án thiết lập quy trình kiểm thử phần mềm chuẩn hóa tham chiếu các nguyên tắc tài liệu kiểm thử theo tiêu chuẩn IEEE 829\cite{ieee829test}. Chiến lược kiểm thử được xây dựng theo mô hình Kim tự tháp kiểm thử (Test Pyramid), bao gồm nhiều cấp độ kiểm tra chặt chẽ:
\begin{itemize}
...
\section{Kiểm thử tự động Backend (Automated Backend Testing)}
...
Toàn bộ \textbf{94 ca kiểm thử tự động} được phân bổ trong 14 bộ kiểm thử (Test Suites) chuyên biệt tại thư mục \texttt{server/tests/}, đạt tỷ lệ vượt qua 100\% (94/94 passed). Bảng~\ref{tab:backend_test_suites} tổng hợp chi tiết kết quả kiểm thử của từng phân hệ:
...
\caption{Bảng tổng hợp kết quả 94 ca kiểm thử tự động backend UniConnect} \label{tab:backend_test_suites} \\
\hline
...
Điểm danh \& Chứng nhận tham gia & \texttt{test\_\allowbreak attendance\_\allowbreak trophy.py} & 6 & Khoảng cách Haversine, sinh \& thẩm định token QR xoay động 30s (HMAC-SHA256), chuẩn hóa mã chứng nhận tham gia hoạt động, chống điểm danh lặp (idempotency), từ chối GPS có sai số phần cứng $acc > 100$m. \\ \hline
...
Phân nhánh dự phòng AI & \texttt{test\_\allowbreak chat\_\allowbreak fallback\_\allowbreak branches.py} & 4 & Cơ chế phân nhánh fallback định tuyến AI nội bộ, xử lý ngoại lệ gián đoạn kết nối Gemini API và phản hồi thông báo an toàn. \\ \hline
...
Luồng tích hợp đầu cuối (E2E) & \texttt{test\_\allowbreak e2e\_\allowbreak critical\_\allowbreak flows.py} & 4 & Vòng đời trọn vẹn: Tạo sự kiện $\rightarrow$ Điểm danh $\rightarrow$ Ghi nhận CTXH; Độc lập giữa xung đột lịch và đăng ký; Ủy nhiệm quyền Co-host nhóm; Xét duyệt vai trò tổ chức kèm audit log. \\ \hline
...
Tải ảnh đại diện Avatar & \texttt{test\_\allowbreak avatar\_\allowbreak upload.py} & 11 & Xác thực định dạng tệp ảnh (JPEG/PNG/WebP), kiểm tra giới hạn dung lượng upload, khử mã độc tệp nhị phân và chuẩn hóa kích thước ảnh đại diện. \\ \hline
Vòng đời Trophy \& Quyền riêng tư & \texttt{test\_\allowbreak trophy\_\allowbreak lifecycle\_\allowbreak and\_privacy.py} & 15 & Quy trình chốt điểm danh (finalize), kiểm tra điều kiện túc số (quorum $\ge 10$), quy trình Admin phê duyệt cấp Trophy, đóng băng sửa điểm danh và bảo mật danh hiệu sự kiện kín. \\ \hline
\multicolumn{2}{|l|}{\textbf{Tổng số ca kiểm thử}} & \textbf{94} & \textbf{Tỷ lệ vượt qua: 94/94 (100\% Passed)} \\ \hline
\end{xltabular}
\endgroup

\subsection{Đặc tả kỹ thuật về Kiểm thử Chu kỳ sống Connection Pool}
...
\section{Kiểm thử tự động Giao diện Frontend (Frontend Component Testing)}
...
\begin{itemize}
    \item \textbf{Kiểm thử xác thực và quản lý token:} Kiểm tra cơ chế lưu trữ và sử dụng Access Token JWT, tự động gắn Bearer Token vào các yêu cầu API và xử lý các trạng thái xác thực không hợp lệ.
...
\section{Đánh giá Tối ưu hóa Hiệu năng Frontend (Bundle Optimization)}
...
\section{Ma trận Kiểm thử Chấp nhận Người dùng (User Acceptance Testing - UAT)}
...
UAT-05 & Người dùng & Đứng tại hội trường ($d \le 100$m, $acc \le 100$m); nhấn "Điểm danh GPS". & Hệ thống tính khoảng cách không gian thỏa mãn; cập nhật \texttt{attendance\_confirmed=True}. & Đạt chuẩn \\ \hline
...
UAT-07 & Người dùng & Hoàn tất điểm danh sự kiện tình nguyện có định mức 0.5 ngày CTXH. & Tự động tích lũy thêm 0.5 ngày CTXH; sau khi chốt điểm danh (túc số $\ge 10$), yêu cầu cấp Trophy chuyển sang \texttt{eligible\_for\_review} chờ Admin duyệt. & Đạt chuẩn \\ \hline
