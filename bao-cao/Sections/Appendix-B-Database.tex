\chapter{Từ điển dữ liệu và Cơ sở dữ liệu chi tiết}
\label{apd:B}
...
\section{Bảng tổng hợp Lược đồ Cơ sở dữ liệu (Database Schema Master Table)}
...
\section{Chi tiết các bảng nghiệp vụ trọng yếu}

\begingroup
\small
\setlength{\parskip}{1pt}

\subsection{Bảng \texttt{activities} (Hoạt động ngoại khóa và sự kiện)}
...
\subsection{Bảng \texttt{join\_requests} (Đăng ký và Xác thực Điểm danh)}
...
\subsection{Bảng \texttt{activity\_cohosts} \& \texttt{activity\_cohost\_invitations} (Đồng tổ chức và Ủy quyền RBAC)}
...
\subsection{Phân hệ Trò chơi hóa - Gamification (\texttt{trophies}, \texttt{user\_trophies}, \texttt{trophy\_grant\_requests})}
