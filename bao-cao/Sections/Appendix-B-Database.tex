...
\texttt{trophies} & trophies/\allowbreak models.py & \texttt{id} & \texttt{activities(id)} & \texttt{name} UNIQUE, \texttt{activity\_id} FK (Nullable), ON DELETE CASCADE \\ \hline
\texttt{user\_\allowbreak trophies} & trophies/\allowbreak models.py & \texttt{id} & \texttt{users(id)}, \texttt{trophies(id)}, \texttt{activities(id)} & UNIQUE (\texttt{user\_id}, \texttt{trophy\_id}), ON DELETE CASCADE \\ \hline
