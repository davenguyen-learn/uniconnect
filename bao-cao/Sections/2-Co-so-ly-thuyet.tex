...
\subsection{Cơ sở mật mã học trong xác thực điểm danh: Thuật toán TOTP và mã phản hồi nhanh QR Code}

Để đảm bảo tính toàn vẹn và chống gian lận trong công tác điểm danh phong trào học đường, hệ thống UniConnect kết hợp các chuẩn mật mã học hiện đại nhằm tạo ra mã xác thực một lần theo thời gian (Time-Based One-Time Password - TOTP).

\subsubsection{Nguyên lý thuật toán TOTP (RFC 6238) và HMAC (RFC 4226)}

Thuật toán TOTP là sự mở rộng của thuật toán sinh mật khẩu một lần dựa trên bộ đếm (HMAC-Based One-Time Password - HOTP)\cite{mraihi2005hotp}. Thay vì sử dụng biến đếm tăng dần $C$, TOTP sử dụng số lượng khoảng thời gian trôi qua kể từ mốc thời gian Unix Epoch ($T_0 = 0$) với kích thước bước thời gian quy định là $T_X$ (trong UniConnect, $T_X = 30$ giây)\cite{krawczyk2011totp}:

\begin{equation}
T = \left\lfloor \frac{\text{CurrentTime} - T_0}{T_X} \right\rfloor
\end{equation}

Giá trị băm xác thực thông điệp có khóa (Keyed-Hash Message Authentication Code - HMAC) được tính toán dựa trên hàm băm mật mã an toàn (SHA-256) kết hợp giữa khóa bí mật $K$ của hoạt động và biến thời gian $T$:

\begin{equation}
\text{HMAC}(K, T) = \text{Hash}\big((K \oplus \text{opad}) \parallel \text{Hash}((K \oplus \text{ipad}) \parallel T)\big)
\end{equation}

Chuỗi băm sau đó được cắt động (Dynamic Truncation) để trích xuất mã token xác thực ngắn hạn. Nhờ đó, mã điểm danh sẽ tự động vô hiệu hóa sau mỗi 30 giây, ngăn chặn triệt để hành vi chụp ảnh màn hình hoặc gửi mã ra ngoài khu vực sự kiện.

\subsubsection{Tiêu chuẩn mã vạch hai chiều QR Code (ISO/IEC 18004)}

Mã QR (Quick Response Code) được chuẩn hóa theo tiêu chuẩn quốc tế ISO/IEC 18004\cite{iso18004qr}. Dữ liệu token TOTP cùng mã định danh hoạt động được mã hóa thành ma trận điểm ảnh vuông 2D tích hợp cơ chế sửa lỗi Reed-Solomon (mức sửa lỗi Medium 15\% hoặc Quartile 25\%). Điều này bảo đảm thiết bị di động của sinh viên có thể nhận diện và quét mã nhanh chóng ngay cả trong điều kiện ánh sáng yếu hoặc góc chụp nghiêng.

\subsection{Thuật toán phát hiện xung đột lịch dựa trên khoảng thời gian nửa mở}
...
\subsubsection{Vị từ giao thoa khoảng thời gian nửa mở $[start, end)$}
