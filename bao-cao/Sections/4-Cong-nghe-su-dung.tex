...
\subsubsection{Cơ chế xác thực và bảo mật (OAuth 2.0 và JWT)}

\textbf{Giới thiệu:}

Để bảo vệ các Endpoint nhạy cảm và xác thực sinh viên, hệ thống áp dụng giao thức ủy quyền OAuth 2.0\cite{hardt2012oauth} kết hợp cùng chuẩn thẻ xác thực JSON Web Token (JWT) theo chuẩn RFC 7519\cite{jones2015jwt}.

\textbf{Đặc điểm thực tế:}

\begin{itemize}
    \item Xác thực phi trạng thái (Stateless Authentication): Máy chủ không cần lưu session trong RAM; mỗi request từ Client mang theo access token có chữ ký số bí mật, giúp hệ thống dễ dàng mở rộng ngang (Horizontal Scaling).
    \item Kiểm soát truy cập dựa trên vai trò (RBAC): Token chứa payload xác định vai trò (Sinh viên, Ban điều hành Nhóm, Quản trị viên), phân quyền chặt chẽ các tác vụ quản trị phong trào theo mô hình RBAC chuẩn mực\cite{sandhu1996role}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{db-container}: Vận hành hệ quản trị cơ sở dữ liệu PostgreSQL 15+, tích hợp sẵn các tiện ích mở rộng pgvector phục vụ lưu trữ vector embedding và PostGIS/Cube hỗ trợ tính toán không gian.
...
\subsubsection{Kiến trúc triển khai thực tế trên nền tảng Đám mây (Cloud Production Architecture)}

Để đưa UniConnect vào vận hành trực tuyến thực tế, phục vụ cộng đồng sinh viên với độ sẵn sàng cao, độ trễ thấp và chi phí hạ tầng tối ưu, đề tài đã hiện thực mô hình triển khai hướng dịch vụ đám mây (Cloud-Native Architecture) kết hợp 4 nền tảng chuyên biệt:

\begin{enumerate}
    \item \textbf{Vercel (Frontend Web Hosting \& CI/CD)}: 
    Ứng dụng giao diện người dùng (React Single Page Application) được đóng gói tối ưu và triển khai trực tuyến trên nền tảng Vercel. Nền tảng này tích hợp quy trình triển khai liên tục (CI/CD) tự động mỗi khi có cập nhật mã nguồn trên nhánh Git, tự động thiết lập chứng chỉ bảo mật HTTPS/TLS và phục vụ ứng dụng web tĩnh với tốc độ phản hồi nhanh.

    \item \textbf{Render (Backend API Container Runtime)}: 
    Khối dịch vụ máy chủ FastAPI được đóng gói thành container và triển khai trực tuyến dưới dạng Web Service trên nền tảng đám mây Render. Nền tảng này quản lý tự động vòng đời ứng dụng, hỗ trợ cơ chế tự phục hồi (Health Check Monitoring), phân phối lưu lượng qua giao thức HTTPS bảo mật và tự động triển khai mã nguồn mới ngay khi có bản cập nhật trên kho lưu trữ Git.

    \item \textbf{Neon Database (Serverless PostgreSQL \& pgvector)}: 
    Toàn bộ dữ liệu quan hệ và chỉ mục vector đa chiều của UniConnect được lưu trữ trên Neon --- nền tảng cơ sở dữ liệu PostgreSQL Serverless hàng đầu hiện nay. Neon cho phép phân tách độc lập giữa tầng lưu trữ (Storage) và tầng tính toán (Compute), hỗ trợ tự động co giãn tài nguyên (Auto-scaling) theo lưu lượng truy cập thực tế, cung cấp tiện ích mở rộng pgvector nguyên bản và tính năng phân nhánh cơ sở dữ liệu (Database Branching) giúp kiểm thử an toàn các kịch bản nâng cấp schema.

    \item \textbf{Supabase Storage (S3-Compatible Object Storage)}: 
    Các dữ liệu tệp tin phi cấu trúc như ảnh đại diện tài khoản (Avatars), hình ảnh minh chứng hoạt động phong trào và các tệp Giấy chứng nhận/Bằng khen điện tử định dạng PDF được lưu trữ tập trung trên Supabase Storage. Backend tương tác với Supabase Storage qua giao thức chuẩn tương thích AWS S3 thông qua thư viện bất đồng bộ \texttt{aioboto3} tại module \texttt{storage.py}, đảm bảo khả năng mở rộng không giới hạn dung lượng, phân quyền truy cập an toàn và truy xuất dữ liệu thông qua đường dẫn công khai (Public URL).
\end{enumerate}

\begin{table}[H]
\centering
\caption{Đối chiếu mô hình triển khai kép (Dual-environment Deployment Matrix) của UniConnect}
\label{tab:dual_deployment_matrix}
\begin{tabular}{|l|p{5.5cm}|p{6.5cm}|}
\hline
\textbf{Tầng hệ thống} & \textbf{Môi trường Cục bộ / Nghiệm thu} & \textbf{Môi trường Vận hành Cloud (Production)} \\ \hline
Frontend & Docker Container (Nginx Alpine SPA) & Vercel (Static Web Hosting, tự động CI/CD) \\ \hline
Backend API & Docker Container (FastAPI + Uvicorn) & Render Cloud Web Service (Managed Container, HTTPS) \\ \hline
Cơ sở dữ liệu & Docker Container (PostgreSQL + pgvector) & Neon Serverless PostgreSQL (Auto-scaling, pgvector) \\ \hline
Lưu trữ đối tượng & Local File System / Volume Mapping & Supabase Storage (S3-compatible, aioboto3) \\ \hline
Trợ lý AI RAG & Gemini 2.5 Flash API & Gemini 2.5 Flash API (Cloud Secret Manager) \\ \hline
\end{tabular}
\end{table}
...
Cuối cùng, chiến lược đóng gói và triển khai kép (Dual-environment Strategy) thông qua Docker Compose ở môi trường thử nghiệm cục bộ và kiến trúc đám mây đa nhà cung cấp (Vercel, Render, Neon, Supabase) trong môi trường sản xuất khẳng định tính sẵn sàng cao của đồ án. Hệ thống vừa đảm bảo tính độc lập, khả năng tái lập 100\% khi bảo vệ trước Hội đồng chấm thi, vừa sẵn sàng vận hành thực tế phục vụ cộng đồng sinh viên với chi phí vận hành tối ưu.